...
for fragment_length in fragment_lengths:
    ...
    for fragment_number in xrange(1,NUMBER_OF_FRAGMENTS_PER_LENGTH+1):
        ...
        # Randomly select one source sequence from the correct class
        # Keep trying until one is chosen from the correct class...
        # If the sequence identifier does not contain 'Qnr' it is
        # assumed that the source sequences are not Qnr and the
        # randomly selected class is discarded.
        # NOTE this might introduce biases/errors, especially if 
        # the source fasta file is a mixed file, especially if
        # it has a skewed content regarding qnr/non-qnr genes...
        random_src_seq = randint(0,len(seqlist)-1)
        while True:
                if random_qnr_class in seqlist[random_src_seq][0]:
                    break
                elif "Qnr" not in seqlist[random_src_seq][0]:
                    break # source sequences are not Qnr
                else:
                    random_src_seq = randint(0,len(seqlist)-1)
...
for index in xrange(0,len(models)):
    ...
    #else:
    # classify the sequences according to different functions
    k = 0.10
    m = 0.0
    print "m\tk\tFN\tFP\tTPR\tFPR\tOBJF"
    for i in xrange(0,300):
        ...
        print m,k, FN, FP, TP/(TP+FN), FP/(FP+TN), TP*(FP+TN)/FP*(TP+FN)
